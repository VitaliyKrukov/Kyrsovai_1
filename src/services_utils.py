...
def search_transactions(list_transactions: list[dict], search_bar: str) -> str:
    """Функция, поиска по слову из описания или категории"""

    if not list_transactions or not search_bar:
        logger.critical("Ошибка проверки если нет списка и входных данных")
        return ""
...
def search_by_phone_number(list_transactions: list[dict]) -> str:
    """Функция, поиска номеров телефона из описания"""
    if not list_transactions:
        logger.critical("Ошибка проверки если нет списка")
        return ""
